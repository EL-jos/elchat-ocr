...
class DynamicFormValidator
{
    ...
    public function validate(ChatbotForm $form, array $values): array
    {
        ...
        foreach ($fields as $field) {
            // Les champs de type "file" ne transitent pas par `values`
            // (gérés séparément via des uploads multipart).
            if ($field->field_type === FormFieldType::File->value) {
                continue;
            }

            $type = FormFieldType::from($field->field_type);

            if (
                $type === FormFieldType::Boolean &&
                isset($values[$field->field_key]) &&
                is_string($values[$field->field_key])
            ) {
                $values[$field->field_key] = match (strtolower(trim($values[$field->field_key]))) {
                    'true' => true,
                    'false' => false,
                    default => $values[$field->field_key],
                };
            }
